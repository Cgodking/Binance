...
python3 binance_futures_zscore_bot.py feasibility-scan --balance 5 --capital-scenarios 5,7.5,10,15,20,25,50,100 --output-dir feasibility_scan_v10
python3 binance_futures_zscore_bot.py v10-replay --days 720 --output-dir momentum_replay_v10
...
- 在不改变 shadow-only 安全状态的前提下，同时反推每个 `symbol × 止损档` 的最低账户权益、最低绝对亏损上限、最低隔离保证金上限和最低名义金额上限，并输出 5、7.5、10、15、20、25、50、100 USDT 的可执行合约曲线。
- 本金探索保持单笔风险比例为 1%，假设研究配置上限随本金同步调整；它只回答订单与风险是否可执行，不代表策略具有正期望。
...
截至 2026-07-18 21:41 UTC 的资本曲线扫描仍分析 530 个合约且无市场数据错误。在保持单笔风险 1% 并同步调整配置上限的假设下，1.0% 止损对应的全市场最低本金约为 6.3002 USDT，SOLUSDT 约为 6.6481 USDT；1.5% 止损下全市场最低约为 8.8002 USDT，SOLUSDT 约为 9.2862 USDT。5 USDT 档没有合约通过 1.0% 止损约束，7.5 USDT 和 10 USDT 档分别有 80 和 83 个满足流动性与执行约束的合约。以上仅为模拟可执行性，不构成入金或实盘建议。

V10 是与 V7/V8/V9 隔离的低换手跨品种动量轮动实验：

- 固定研究 SOLUSDT、BNBUSDT、XRPUSDT、DOGEUSDT、ADAUSDT、ZECUSDT，不根据测试期结果动态更换品种。
- 每日仅在 UTC 日线边界使用已闭合 4 小时 K 线计算 7 日与 30 日风险调整动量，并要求价格方向与 EMA50 一致。
- 在至少四个品种同时具备有效特征时，选择绝对动量分数最高的一个方向；全组合最多持有一个虚拟仓位，信号在下一根 4 小时 K 线开盘成交。
- 止损为 1.5 ATR 并限制在 1%-3%，目标为 2.5R，最长持仓 7 天；轮动、信号失效、止损、止盈或超时均可退出。
- 固定 100 USDT 账本只判断信号边际，5、7.5、10、15 USDT 风险账本分别计算交易所过滤器下的执行覆盖率；10 USDT 是预先指定的资格本金档。
- 720 天回放使用 180 天训练、90 天验证、90 天测试并每 90 天滚动，边界隔离 7 天。必须同时通过训练、验证、测试、测试窗口一致性、1.5 倍成本、块自助置信区间及 10 USDT 资本覆盖门槛。
- 历史通过最多只能进入新的 forward shadow 验证，不会自动开启 `FORWARD_SHADOW_EXECUTION_ENABLED`，也不产生实盘资格。

截至 2026-07-18 22:20 UTC 的 720 天 V10 冻结回放未通过历史门槛。100 USDT 研究账本中，训练、验证、测试分别完成 60、40、52 笔，净收益分别为 19.9259、7.3704、8.0356 USDT，但净利润因子仅为 1.1676、1.0992、1.0845，均低于预设的 1.20、1.15、1.15 门槛。5 个滚动测试窗口中 4 个为正，但 1.5 倍成本下测试净利润因子降至 1.0402，2 倍成本下净收益转为 -0.1969 USDT；测试期块自助法单笔期望 95% 区间为 -0.6104 至 0.9888 USDT，下界没有高于零。方向结果也不稳定：Long 29 笔净亏损 9.6591 USDT，Short 23 笔净盈利 17.6947 USDT；SOLUSDT 单品种 13 笔净亏损 26.8416 USDT。按账户权益 1% 的单笔风险预算和实时交易所数量过滤器重算后，5、7.5、10 USDT 三档均为 0/52 笔可执行，15 USDT 仅 3/52 笔可执行。V10 被标记为 `NOT_VIABLE`，不能进入 forward shadow，结论保持 `DO_NOT_TRADE_LIVE`。
