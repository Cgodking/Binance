...
- 使用闭合 15 分钟 K 线确认趋势，使用闭合 5 分钟 K 线确认 ATR 自适应回调，并在下一根 5 分钟 K 线开盘模拟成交。
- 120 天训练、30 天验证、30 天测试，每次向前滚动 30 天。
- 每个训练/验证边界使用 240 分钟隔离区，600 天数据可形成 15 个测试窗口。
...
- 止盈、止损使用 5 分钟 ATR 自适应距离，并设置上下界，避免低波动时执行成本吞噬目标收益。
